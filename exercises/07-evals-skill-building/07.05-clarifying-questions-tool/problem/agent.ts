...
const tools = {
	...
	askForClarification: tool({
		description:
			"Ask clarifying questions when a user's request is missing critical information needed to complete the task. Use 'select' inputType with options for questions with enumerable answers (cities, dates, priorities). Use 'text' inputType for open-ended questions (file paths, message content, custom text).",
			...
			questions: z
				.array(
					z.object({
						...
						field: z
							.string()
							.describe("The field name this question relates to"),
						inputType: z
							.enum(["select", "text", "date", "number"])
							.default("select")
							.describe(
								"The type of input: 'select' for enumerable choices, 'text' for free-form input, 'date' for date picker, 'number' for numeric input",
							),
						options: z
							.array(z.string())
							.optional()
							.describe(
								"Pre-filled answer choices. Required when inputType is 'select', omit for other input types.",
							),
					}),
				)
				.describe("Array of clarifying questions to ask the user"),
		}),
		execute: async () => {
	...
	createSpreadsheet: tool({
		description: "Create new spreadsheet with columns and initial data",
		inputSchema: z.object({
			name: z.string().describe("Name of spreadsheet"),
			columns: z.array(z.string()).describe("Column headers"),
			initialRows: z.number().optional().describe("Number of empty rows"),
		}),
	...
	updateCalendarEvent: tool({
		description: "Update existing calendar event time, location, or attendees",
		inputSchema: z.object({
	...
	searchWeb: tool({
		description: "Search web for current information and return top results",
		inputSchema: z.object({
	...
	translateText: tool({
		description: "Translate text between languages with language detection",
		inputSchema: z.object({
	...
	createTask: tool({
		description: "Create task in todo list with priority and due date",
		inputSchema: z.object({
			title: z.string(),
	...
	analyzeImage: tool({
		description: "Analyze image to detect objects, text, or describe contents",
		inputSchema: z.object({
			imageUrl: z.string().url().describe("Image URL"),
			analysisType: z.enum(["objects", "text", "description", "faces"]),
	...
	bookFlight: tool({
		description: "Search and book flights between cities with preferences",
		inputSchema: z.object({
	...
	generateQRCode: tool({
		description: "Generate QR code for URL, text, or contact information",
		inputSchema: z.object({
	...
	setReminder: tool({
		description: "Set reminder notification for specific date and time",
		inputSchema: z.object({
	...
	convertCurrency: tool({
		...
		inputSchema: z.object({
			...
			date: z.string().optional().describe("Historical date for rates"),
		}),
		execute: async () => "Currency conversion completed successfully",
	}),

	transcribeAudio: tool({
		description: "Transcribe audio file to text with speaker detection",
		inputSchema: z.object({
		...
		execute: async () => "Audio transcription completed successfully",
	}),
	...
	checkWeather: tool({
		...
		inputSchema: z.object({
			...
			days: z.number().default(1).describe("Forecast days (1-14)"),
	...
	compressFile: tool({
		...
		execute: async () => "File compression completed successfully",
	}),

	schedulePost: tool({
		description: "Schedule social media post to multiple platforms",
		inputSchema: z.object({
	...
	analyzeDocument: tool({
		...
		inputSchema: z.object({
			documentUrl: z.string().url().describe("Document URL (PDF, DOCX, etc)"),
			extractTables: z.boolean().default(true),
			extractImages: z.boolean().default(false),
			summarize: z.boolean().default(false),
		}),
		execute: async () => "Document analysis completed successfully",
	}),

	createBackup: tool({
		description: "Create backup of specified files or databases to storage",
		inputSchema: z.object({
			sources: z.array(z.string()).describe("Paths to backup"),
			destination: z.string().describe("Backup destination path or URL"),
			incremental: z.boolean().default(false),
			encrypt: z.boolean().default(true),
			retention: z.number().optional().describe("Days to keep backups"),
		}),
	...
	generateReport: tool({
		...
		inputSchema: z.object({
			...
			reportType: z.enum(["sales", "financial", "performance", "custom"]),
			dateRange: z.object({
				start: z.string(),
				end: z.string(),
			}),
	...
	moderateContent: tool({
		...
		execute: async () => "Content moderation completed successfully",
	}),
};
...
When a user's request is missing critical information needed to complete the task, use the askForClarification tool. Choose the appropriate inputType for each question:

- "select": Use when answers are enumerable (cities, priorities, date ranges, platforms). MUST include options array.
- "text": Use for open-ended input (file paths, message content, custom text, the actual text to translate).
- "date": Use for specific date input.
- "number": Use for numeric input (quantities, amounts).
...
{
  "questions": [
    { "question": "Where are you departing from?", "field": "from", "inputType": "select", "options": ["New York", "Los Angeles", "Chicago", "San Francisco"] },
    { "question": "When do you want to depart?", "field": "departDate", "inputType": "date" },
    { "question": "How many passengers?", "field": "passengers", "inputType": "number" }
  ]
}
...
{
  "questions": [
    { "question": "What text would you like to translate?", "field": "text", "inputType": "text" },
    { "question": "What language should it be translated to?", "field": "targetLanguage", "inputType": "select", "options": ["Spanish", "French", "German", "Japanese", "Chinese"] }
  ]
}
...
{
  "questions": [
    { "question": "What is the path to the file or folder?", "field": "sourcePath", "inputType": "text" },
    { "question": "Where should the archive be saved?", "field": "outputPath", "inputType": "text" },
    { "question": "What format?", "field": "format", "inputType": "select", "options": ["zip", "tar", "gz", "7z"] }
  ]
}

4. "Send John an email"
{
  "questions": [
    { "question": "What is John's email address?", "field": "to", "inputType": "text" },
    { "question": "What should the subject be?", "field": "subject", "inputType": "text" },
    { "question": "What would you like to say?", "field": "body", "inputType": "text" }
  ]
}

Do NOT call other tools if critical information is missing - use askForClarification first.
Do NOT ask for clarification if the request already has all required information.`,
	});
