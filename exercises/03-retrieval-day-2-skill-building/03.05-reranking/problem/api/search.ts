...
export const searchChunks = async (opts: {
	...
	const bm25Map = new Map(bm25SearchResults.map((r) => [r.chunk, r.score]));
	...
	const topResultsForReranking = rrfResults.slice(0, opts.rerankCount);

	// Assign IDs to chunks for reranking
	const topResultsWithId = topResultsForReranking.map((result, i) => ({
		...result,
		id: i,
	}));
	...
	const chunksWithId = topResultsWithId
		.map((result) =>
			[`## ID: ${result.id}`, `<content>`, result.chunk, `</content>`].join(
				"\n\n",
			),
		)
		.join("\n\n");

	const searchQuery = [opts.keywordsForBM25?.join(" "), opts.embeddingsQuery]
		.filter(Boolean)
		.join(" ");
	...
	const rerankedResults = await generateObject({
		schema: z.object({
			resultIds: z
				.array(z.number())
				.min(1)
				.describe("Array of IDs of the most relevant chunks"),
		}),
		model: google("gemini-2.5-flash-lite"),
		system: `
      You are an expert in retrieval re-ranking. You are given a search query and a set of chunks. Your task is to select the most relevant chunks based on the search query. You should return only the IDs of the most relevant chunks, not the full chunks. You should also be selective and only include chunks that are genuinely helpful for answering the question. If a chunk is only tangentially related or not relevant, exclude its ID. If you are unsure, exclude its ID.
    `,
		prompt: `
      Search query:
